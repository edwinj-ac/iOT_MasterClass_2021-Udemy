...
$conn = mysqli_connect("localhost","admin_cursoiot","161616","admin_cursoiot");
          ...
          <div class="nav-fold">
            <a href="profile.html">
              ...
              <span class="clear hidden-folded p-x">
                <span class="block _500">Edwin Jesus Aguilar Cortés</span>
...
//Funciones para enviar datos al broker desde un compotente switch de la pagina web
function process_led1(){
  if ($('#input_led1').is(":checked")){
    console.log("Encendido");

    client.publish('led1', 'on', (error) => { //topic: led1, mensaje: on
...
const WebSocket_URL = 'wss://spac3r.tk:8094/mqtt'
const client = mqtt.connect(WebSocket_URL, options)

client.on('connect', (error) => {
console.log('Mqqt conectado por WebSocket! Exito!', error)

//Enviar mensaje al EMQX
/*  client.publish('fabrica', 'esto es un verdadero exito', (error) => { //client.publish(topic, message)
  console.log(error|| 'Publish Success')
})*/
  <?php foreach ($devices as $device) { ?>

    client.subscribe('<?php echo $device['devices_serie'] ?>/access_query', {q:0}, (error) => {
      /*if(!error){
          console.log('Suscripción exitosa!')
        }else{
          console.log('Suscripción fallida :c')
        }*/
        })
      client.subscribe('<?php echo $device['devices_serie'] ?>/temp', {q:0}, (error) => { })
  <?php }?>
})
